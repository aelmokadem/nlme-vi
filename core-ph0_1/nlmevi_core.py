...
DEVICE = torch.device("cpu")  # these models are too small for a GPU to help
...
class OneCmtIVBolusMM:
    ...
    def __init__(self, dose, grid_np, obs_idx_np):
        self.dose = dose
        self.t_grid = torch.tensor(grid_np)
...
def fit_model(model, data, kind, family, K, theta_init, n_eta, n_steps=4000,
             ...
             min_steps=2000, tol=0.01, max_steps=25000):
    """
    ...
    Returns (theta_np, q, ll, ess, top_share, n_steps_run, converged).
    """
    ...
    ll, ess, top_share = is_marginal_loglik(model, q, theta.detach(), batch)
    return theta.detach().cpu().numpy(), q, ll, ess, top_share, n_run, converged
