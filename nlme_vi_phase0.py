...
def summarize(df):
    ...
    # df is long (one row per parameter per fit), so dedupe to one row per
    # actual fit before aggregating runtime/step-count -- otherwise secs
    # gets counted 7x (once per parameter name).
    per_fit = df.drop_duplicates(["replicate", "posterior", "K"])
    print("\n" + "-" * 72)
    print("Runtime -- cpu_secs is the number a VI-vs-FOCEI/SAEM speed claim should")
    print("rest on (time.process_time(), immune to OS sleep and largely immune to")
    print("other-process contention). secs (wall-clock) is shown alongside for")
    print("context but can be inflated by anything that paused or slowed the machine")
    print("during the run -- a >3x gap between mean cpu_secs and mean secs for the")
    print("same cell is a sign the wall-clock number for that cell is contaminated.")
    rt = per_fit.groupby(["posterior", "K"])[["cpu_secs", "secs", "n_steps_run"]].agg(
        ["mean", "median", "min", "max"])
    print(rt.round(2).to_string())
    total_cpu = per_fit.cpu_secs.sum()
    total_secs = per_fit.secs.sum()
    ...
    if total_secs > 1.5 * total_cpu:
        print(f"  *** wall time is {total_secs/total_cpu:.1f}x cpu time -- this run "
              f"was likely interrupted (sleep) or contended (other processes). "
              f"Use cpu_secs, not secs, for any reported benchmark from this run. ***")
    print(f"\n  NOTE for any VI-vs-FOCEI/SAEM speed comparison: pin thread counts on "
          f"both sides (torch.set_num_threads(N) here; check nlmixr2/your BLAS's "
          f"threading) even when using cpu_secs -- otherwise the comparison still "
          f"conflates the algorithm with incidental hardware parallelism.")
...
# %% ------------------------------------------------------------- config
# Plain top-level variables, not an argparse namespace -- run this cell,
# then change any of these directly and re-run downstream cells, same as
# running `python nlme_vi_phase0.py --flag value` from the command line.
# parse_known_args() makes this safe to run inside VSCode/Spyder/Jupyter's
# interactive kernel too (it ignores whatever extra args the kernel itself
# was launched with, instead of erroring on them).
...
print(f"config: N_SUBJ={N_SUBJ}  N_REPS={N_REPS}  K_GRID={K_GRID}  "
     f"MAX_STEPS={MAX_STEPS}  DREP={DREP}  OUT={OUT}")


# %% ------------------------------------------------------------ sanity
# Run this cell on its own and inspect `sanity_ok` before going further --
# nothing downstream is trustworthy if this is False.
sanity_ok = sanity_checks(tp)
if not sanity_ok:
    print("*** SANITY CHECKS FAILED -- fix before running the experiment cell ***")


# %% --------------------------------------------------------- experiment
# The expensive cell. Re-run just this one if you change N_REPS/K_GRID/etc.
# above without wanting to re-verify sanity checks every time.
assert sanity_ok, "sanity checks failed -- see the cell above"

print("=" * 72)
print(f"EXPERIMENT  N={N_SUBJ} subjects x {len(TIMES)} obs, "
      f"{N_REPS} replicates, K in {K_GRID}, drep={DREP}")
print(f"TRUE omegas: CL={tp.om_CL}  V={tp.om_V}  ka={tp.om_ka}")
print("=" * 72)

df = run_experiment(tp, N_SUBJ, TIMES, N_REPS, K_GRID,
                    ["free", "amortized"], N_STEPS, drep=DREP,
                    max_steps=MAX_STEPS)
# `df` is now sitting in your interactive session -- inspect it directly,
# e.g. df.head(), df.query("posterior=='amortized' and K==1"), etc.
...
# %% ---------------------------------------------------- save + figure
df.to_csv(f"{OUT}/phase0_results.csv", index=False)
make_figure(df, f"{OUT}/phase0_omega_bias.png")
print(f"Raw results -> {OUT}/phase0_results.csv")
