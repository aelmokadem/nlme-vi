...
dat <- read_csv(input_csv, show_col_types = FALSE,
                na = c("", "NA", "."))
# na="." is required: write_nonmem_csv (Python side) writes DV="." for
# dosing rows (standard NONMEM convention for "no observation here").
# readr's default na values are only "" and "NA" -- without this, the
# literal string "." makes the whole DV column parse as character instead
# of numeric, and the fit fails (or silently misbehaves) downstream.
...
elapsed_wall <- as.numeric(Sys.time() - t0_wall, units = "secs")
cpu_delta <- proc.time() - t0_cpu
# user.self + sys.self = CPU seconds actually consumed by THIS R process
# (analogous to Python's time.process_time() on the VI side). Unlike
# Sys.time()-based wall-clock, this does not advance during OS sleep and is
# far less sensitive to other-process contention -- use this for any
# VI-vs-FOCEI/SAEM speed comparison, not elapsed_wall.
elapsed_cpu <- unname(cpu_delta["user.self"] + cpu_delta["sys.self"])
cat(sprintf("Fit completed: cpu %.1fs / wall %.1fs\n", elapsed_cpu, elapsed_wall))
