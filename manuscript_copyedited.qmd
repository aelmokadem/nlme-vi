...
On the linear one-compartment model with known ground truth, standard-ELBO ($K=1$) fitting systematically underestimates between-subject variability under both free and amortized posteriors, with the bias substantially reduced by importance weighting at larger $K$ (@fig-phase0-omega-bias and @supptbl-phase0-headline). Structural fixed effects show much less dependence on $K$. Clearance and volume remain comparatively stable, whereas $k_a$ is consistently underestimated by a similar amount across $K$, indicating a persistent bias that is distinct from the strongly $K$-dependent behavior of the random-effect parameters. Residual-error estimates show greater $K$-dependence in some settings, consistent with partial compensation between residual and between-subject variability (@supptbl-phase0-fixed-effects; @suppfig-phase0-fixed-effects).
...
design (@fig-phase1-grid and @supptbl-phase1-q2q4). Structural fixed-effect estimates are substantially less sensitive to $K$ across the same grid, whereas residual-error estimates show greater dependence on $K$ in some panels, consistent with redistribution of unexplained variability between residual error and between-subject random effects (@supptbl-phase1-q2q4-fixed-effects; @suppfig-phase1-fixed-effects).
...
::: {#supptbl-phase0-headline}
{{< include publication/tables/table_phase0_headline.md >}}

Mean relative bias (%) in random-effect standard deviations ($\omega$) by posterior architecture and $K$.
:::

::: {#supptbl-phase0-fixed-effects}
{{< include publication/tables/table_phase0_fixed_effects.md >}}

Mean relative bias (%) in structural fixed effects and residual error by posterior architecture and $K$. This companion table distinguishes persistent or residual-error bias from the strongly $K$-dependent bias in between-subject variability.
:::

::: {#supptbl-phase1-q2q4}
{{< include publication/tables/table_phase1_q2q4.md >}}

Mean relative bias (%) in random-effect standard deviations ($\omega$) by sampling design, posterior architecture, variational family, and $K$.
:::

::: {#supptbl-phase1-q2q4-fixed-effects}
{{< include publication/tables/table_phase1_q2q4_fixed_effects.md >}}

Mean relative bias (%) in structural fixed effects and residual error using the same grouping as the primary Phase 1 analysis.
:::

::: {#supptbl-phase1-q3-nonlinear}
{{< include publication/tables/table_phase1_q3_nonlinear.md >}}

Mean parameter estimates, ground-truth values, relative bias (%), and converged fraction for the Michaelis-Menten elimination model.
:::
